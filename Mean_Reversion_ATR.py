...
breeze = BreezeConnect(api_key="95r1e22!L04=81693j2%710*f543I13y")
import urllib
breeze.generate_session(api_secret="9i%+v203Q1T3R24Vm303~13BepZ90831",
                        session_token="47252927")
    ...
    if t(9, 30)<t(datetime.now().time().hour, datetime.now().time().minute)<t(15, 30) :
        ...
        if order == 1 :
            ...
            if exit_premium <= SL or (t(datetime.now().time().hour, datetime.now().time().minute) == t(15,20)) :
                ...
                csv_file = "Mean_reversion_ATR.csv"
                try:
                    with open(csv_file, 'x', newline='') as file:
                        writer = csv.writer(file)
                        writer.writerow(['Date', 'Entry Time', 'Strike', 'CE/PE','Entry Premium', 'Exit Time', 'Exit premium', 'PnL'])
